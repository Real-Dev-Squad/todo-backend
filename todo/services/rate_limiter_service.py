...
class RateLimiterService:
    ...
    def _cleanup_old_windows(self, identifier: str, rule_name: str, current_time: datetime):
        ...
        try:
            ...
            self.collection.delete_many(
                {"identifier": identifier, "rule_name": rule_name, "window_start": {"$lt": cutoff_time}}
            )
        except Exception as e:
        ...
        try:
            ...
            windows = list(
                self.collection.find(
                    {"identifier": identifier, "rule_name": rule_name, "window_start": {"$gte": cutoff_time}}
                ).sort("window_start", 1)
            )
    ...
    def _update_window_count(self, identifier: str, rule_name: str, current_time: datetime):
        ...
        try:
            ...
            result = self.collection.update_one(
                {"identifier": identifier, "rule_name": rule_name, "window_start": current_window_start},
                {"$inc": {"request_count": 1}, "$set": {"last_request_time": current_time}},
                upsert=True,
            )

            if result.upserted_id:
                # New window created, set created_at
                self.collection.update_one({"_id": result.upserted_id}, {"$set": {"created_at": current_time}})
                ...
                try:
                    self.dual_write_service.create_document("rate_limit_cache", window_data, str(result.upserted_id))
                except Exception as e:
    ...
    def update_rule(self, rule_name: str, **kwargs) -> bool:
        ...
        try:
            ...
            result = self.rules_collection.update_one({"name": rule_name}, {"$set": update_data}, upsert=True)
            ...
            if result.upserted_id or result.modified_count > 0:
                ...
                if rule_data:
                    ...
                    # Sync to PostgreSQL using dual-write service
                    self.dual_write_service.create_document("rate_limit_rules", rule_data, mongo_id)
