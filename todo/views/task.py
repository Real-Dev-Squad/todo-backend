...
class TaskListView(APIView):
    @extend_schema(
        ...
        description="Retrieve a paginated list of tasks with optional filtering and sorting. Use profile=true to get only current user's tasks.",
        tags=["tasks"],
        parameters=[
            OpenApiParameter(
                name="page",
            ...
            OpenApiParameter(
                ...
                location=OpenApiParameter.QUERY,
                description="Field to sort by (createdAt, updatedAt, priority, dueAt)",
                required=False,
            ),
            OpenApiParameter(
                name="order",
            ...
            OpenApiParameter(
                ...
                location=OpenApiParameter.QUERY,
                description="If true, returns only tasks assigned to current user",
                required=False,
            ),
        ],
        responses={
    ...
    @extend_schema(
        ...
        description="Create a new task with the provided details. The task can be assigned to a user or team.",
        tags=["tasks"],
        request=CreateTaskSerializer,
        responses={
            201: CreateTaskResponse,
            400: OpenApiResponse(description="Bad request - Invalid task data"),
            401: OpenApiResponse(description="Unauthorized - Authentication required"),
            500: OpenApiResponse(description="Internal server error"),
        },
    )
...
class TaskDetailView(APIView):
    @extend_schema(
        ...
        description="Retrieve a single task by its unique identifier with full details including assignee information",
        tags=["tasks"],
        parameters=[
            OpenApiParameter(
                name="task_id",
                ...
                description="Unique identifier of the task (ObjectId format)",
                required=True,
            ),
        ],
        responses={
            200: GetTaskByIdResponse,
            400: OpenApiResponse(description="Bad request - Invalid task ID format"),
    ...
    @extend_schema(
        ...
        summary="Delete task",
        description="Delete a task by its unique identifier. Only the task creator or assignee can delete the task.",
        tags=["tasks"],
        parameters=[
            OpenApiParameter(
                name="task_id",
                type=OpenApiTypes.STR,
                location=OpenApiParameter.PATH,
                description="Unique identifier of the task to delete (ObjectId format)",
                required=True,
            ),
        ],
        responses={
            ...
            400: OpenApiResponse(description="Bad request - Invalid task ID format"),
            401: OpenApiResponse(description="Unauthorized - Authentication required"),
            403: OpenApiResponse(description="Forbidden - Insufficient permissions to delete task"),
            404: OpenApiResponse(description="Task not found"),
            500: OpenApiResponse(description="Internal server error"),
        },
    )
    ...
    @extend_schema(
        ...
        description="Partially update a task or defer it based on the action parameter. Only the task creator or assignee can update the task.",
        tags=["tasks"],
        parameters=[
            OpenApiParameter(
                name="task_id",
        ...
        responses={
            200: TaskDTO,
            400: OpenApiResponse(description="Bad request - Invalid task data or action"),
            401: OpenApiResponse(description="Unauthorized - Authentication required"),
            403: OpenApiResponse(description="Forbidden - Insufficient permissions to update task"),
            404: OpenApiResponse(description="Task not found"),
            500: OpenApiResponse(description="Internal server error"),
        },
    )
