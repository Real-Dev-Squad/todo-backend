...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "action", choices=["show", "update", "create", "delete"], help="Action to perform on rate limiting rules"
        )
        parser.add_argument("--rule-name", default="default", help="Name of the rate limiting rule (default: default)")
        parser.add_argument("--window-size", type=int, help="Window size in minutes")
        parser.add_argument("--num-windows", type=int, help="Number of windows")
        parser.add_argument("--requests-per-second", type=int, help="Maximum requests per second")
        parser.add_argument("--sliding-scale-factor", type=float, help="Sliding scale factor (0.0 to 1.0)")
        parser.add_argument("--active", type=bool, help="Whether the rule is active (true/false)")
        parser.add_argument(
            "--output-format", choices=["json", "table"], default="table", help="Output format for show command"
        )
    ...
    def update_rule(self, rule_name, options):
        ...
        try:
            ...
            if success:
                self.stdout.write(self.style.SUCCESS(f"Successfully updated rule '{rule_name}'"))
                # Show updated rule
    ...
    def create_rule(self, rule_name, options):
        ...
        try:
            ...
            if success:
                self.stdout.write(self.style.SUCCESS(f"Successfully created rule '{rule_name}'"))
                # Show created rule
                self.show_rule(rule_name, "table")
            else:
    ...
    def delete_rule(self, rule_name):
        ...
        try:
            ...
            if success:
                self.stdout.write(self.style.SUCCESS(f"Successfully deactivated rule '{rule_name}'"))
            else:
